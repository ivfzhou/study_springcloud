...
public class TokenBucket {
    ...
    // 尝试去获取tokens个令牌，返回值表示获取这些令牌，需要等待的时间，如果返回0表示无需等待。
    ...
    // 尝试预约tokens令牌，如果在timeout时间范围内，可以预约到，就返回true（需要等待预约的时间）。
    // 如果发现timeout时间范围内，没办法预约到当前令牌，直接返回false，表示令牌获取失败（无需等待）。
    ...
    // 直接获取指定数量的令牌，如果可以直接拿到，返回true，如果不能直接拿，直接返回false。
